...
%   esparciendose para cada linea de pixeles desde el centro del target         > i = Video de an�lisis.
%   hasta el exterior. La informaci�n contenida en la variable "Videos" 
...
%   esparcimiento de todos los videos de un fen�meno.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
load('videos/vid_g48.mat') 
 for i_ = 1 :length(Partes)                                                     % For para los Videos 
        for j_ = 1 : length({Partes{i_,3}.Radios})-3                            % for para los instantes
            ...
            lineas_ =  videos{i_, j_,2};                                          % Copio todas las l�neas de 1 video i_ del instante j_ aparte para analizar
            
            for line_ = 1:length(lineas_(1).fotos)                              % Bucle para las l�neas
  ...
  for i_ = 1 :length(Partes)                                                    % For para los Videos  
    for j_ = 1 : length({Partes{i_,3}.Radios})-3                                % for para los instantes
        for fotogrm_ = 1 : 3                                                    % Bucle para Fotogramas
            [L_, W_] = size(videos{i_, j_}(1).fotos);                           % Largo y ancho de la imagen de todas las l�neas
            plot_fig(L_,W_) = uint8(1);                                         % Creo imagen negra del tama�o de la imagen
