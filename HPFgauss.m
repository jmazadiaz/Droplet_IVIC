...
%   frecuencias baja usando una ecuación de tranasferencia cuya forma es
...
function I13 = HPFgauss(I,sigma,show)
...
%I=rgb2gray(imread('D:\Droplets Video\agua\48\AVI\IMG\img-A48-1\f001.jpg'));
    ...
    H_gau = 1 -  exp(-(dist .^2) / (2 * (sigma ^2)));                           % Creamos la base del filtro, arreglo de zeros del tamaño de la imagen.
                    
    DFT_filt_gau = H_gau .* I_dft;                                              % Operamos, cambian a cero todo lo que esta fuera del radio
    I13 = real(ifft2(DFT_filt_gau));                                             % Hacemos la tranasformada inversa para convertir la imagen del dom de frecuencia al Dom espacial
...
if show == true                                                                 % En caso de visaulizar el resultado
    ...
    subplot(2,3,4), imshow(I13), title('Filtered Image');
    subplot(2,3,6), imshow(fftshift(H_gau)), title('Gaussian higt-pass filter');
