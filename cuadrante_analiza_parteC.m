...
%   para cada linea de pixeles desde el centro del target hasta el              > i = Video de an�lisis.
%   exterior para as� ser medida aparte. La informaci�n contenida en 
...
%     filebw = strcat(folfer,'bw_vid_g48')
 for i_ = 1 :length(Partes)                                                     % Video
     ...
     Num_line_ =  length({Partes{i_,3}(j_).Radios.L1});                         % N�mero de l�neas
     Num_pxline_ = length(Partes{i_,3}(j_).Radios(1).L1);                       % Numero de pixels de cada l�nea
