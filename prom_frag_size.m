...
if calcula_ == true
 clear dec_frag_ elmt_frag_ frag_med_ frag_size_ med_desv_ Partes
    ...
    for caso_ = 1:length(file_partes_)   
        ...
        frag_size_(caso_,1:3) = {file_partes_(caso_).name(8:11),str2num(file_partes_(caso_).name(10:11)), table_,};
...
for indice_ = 1 :length(frag_size_) 
    ...
    temp_elmt_frag_ = elmt_frag_(indice_,:);
    
    for ind_ = 1:length(temp_elmt_frag_)
      if isempty(cell2mat(temp_elmt_frag_(ind_)))
        break
      elseif max(cell2mat(temp_elmt_frag_(2))) ~=0
        std_elmt_frag_(indice_,ind_) = std(cell2mat(temp_elmt_frag_(ind_)));
        mean_elmt_frag_(indice_,ind_) = mean(cell2mat(temp_elmt_frag_(ind_)));
        median_elmt_frag_(indice_,ind_) = median(cell2mat(temp_elmt_frag_(ind_)));
      end
    end
